...
if __name__ == "__main__":
    ...
    ids=set(list(io_process.read_json("{}".format(args.ids_file)).keys())) # TODO: the difference
    ids=[each_id.split("#")[0] for each_id in ids]
    ...
    for id in ids:
        video_path = os.path.join(video_root_path, "{}.avi".format(id))
        video_id =id
        feature_path = os.path.join(feature_root_path, "{}.npy".format(video_id))
        line = ",".join([video_path, feature_path])
        fp_wt.write(line + "\n")
